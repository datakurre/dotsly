...
const missingInPalette2D = [...colorsInPalette].filter(rgb => !colorsInPalette2D.has(rgb));
const extraInPalette2D = [...colorsInPalette2D].filter(rgb => !colorsInPalette.has(rgb));
...
console.log('✓ Color sources are consistent:', missingInPalette2D.length === 0 && extraInPalette2D.length === 0);